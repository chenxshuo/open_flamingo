"""
Copied from https://github.com/allenai/mmc4/blob/main/scripts/download_images.py
...
def parse_args():
    ...
    parser.add_argument('--output_image_dir', type=str, default=None,
                        help='Local path to the directory that stores the downloaded images')
...
def download_images_multiprocess(args, df, func):
    ...
    with shelve.open(shelve_filename) as results:
        ...
        pool_data = ((index, df[i:i + chunk_size], func) for index, i in enumerate(range(0, len(df), chunk_size)) if
                     index not in finished_chunks)
