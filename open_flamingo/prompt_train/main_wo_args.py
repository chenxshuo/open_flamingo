...
parser.add_argument("--model_type", type=str, default="3B", help="Model type to use, 3B or 9B", choices=["3B", "9B"])
parser.add_argument("--bs", type=int, default=8, help="Batch size")
parser.add_argument("--number_of_classes", type=int, default=16, help="Number of classes to train on", choices=[8, 16, 32])
parser.add_argument("--number_of_media_prompts", type=int, default=5, help="Number of media prompts per class")
parser.add_argument("--number_of_text_prompts_per_media", type=int, default=3, help="Number of text prompts per media prompt")
parser.add_argument("--epochs", type=int, default=200, help="Number of epochs to train")
parser.add_argument("--use_robust_prompting", type=bool, default=True, help="Use robust prompting")
parser.add_argument("--robust_scales", type=list, nargs="+", default=[224, 299, 384], help="Robust scales")
parser.add_argument("--evaluate_dataset", type=str, default="imagenet-a", help="Dataset to evaluate on", choices=["imagenet-1k", "imagenet-a"])
parser.add_argument("--evaluation_mode", type=str, default="classification", help="Evaluation mode", choices=["generation", "classification"])
parser.add_argument("--only_load_and_eval", type=bool, default=False, help="Only load and evaluate")
...
def get_multi_scales_vision_tensor(image, image_processor, scales):
    vision_x = []
    for scale in scales:
        img_resize_fn = T.Compose([T.Resize((scale, scale), interpolation=T.InterpolationMode.BICUBIC)])
        vision_x.append(image_processor(img_resize_fn(image)).unsqueeze(0))
    vision_x = torch.cat(vision_x, dim=0)
    vision_x = vision_x.unsqueeze(1).unsqueeze(0)
    return vision_x

def build_train_prompt_sentence(number_of_media_tokens, number_of_text_tokens_per_media, query_label):
...
def prepare_one_training_batch(batch, number_of_media_tokens, number_of_text_tokens_per_media, tokenizer, image_processor, use_robust_prompting=False, robust_scales=[]):
    ...
    for img, class_name in zip(batch_image, batch_class_name):
        prompt_sentence = build_train_prompt_sentence(number_of_media_tokens, number_of_text_tokens_per_media, class_name)
        ...
        if use_robust_prompting:
            vision_x = get_multi_scales_vision_tensor(img, image_processor, robust_scales)
    ...
    lang_x = tokenizer(batch_lang, return_tensors="pt", padding="longest", truncation="only_first", max_length=128)
    # logger.info(f"vision_x: {vision_x.shape}, lang_x: {lang_x['input_ids'].shape}")
    # assert False
    return vision_x, lang_x


def prepare_one_eval_batch(batch, number_of_media_tokens, number_of_text_tokens_per_media, tokenizer, image_processor, use_robust_prompting=False, robust_scales=[]):
    ...
    for img  in batch_image:
        prompt_sentence = build_eval_prompt_sentence(number_of_media_tokens, number_of_text_tokens_per_media)
        # logger.debug(f"prompt_sentence: {prompt_sentence}")
        if use_robust_prompting:
            vision_x = get_multi_scales_vision_tensor(img, image_processor, robust_scales)
            batch_vision_tensor.append(vision_x)
...
def get_val_data_loader(dataset_name, batch_size, num_workers, number_of_classes):
    assert dataset_name in ["imagenet-1k", "imagenet-a"], "Only imagenet-1k and imagenet-a are supported for now."
    if dataset_name == "imagenet-1k":
        ...
        raise NotImplementedError("Only imagenet-1k and imagenet-a are supported for now.")
...
if __name__ == "__main__":
    ...
    assert EVALUATE_DATASET in ["imagenet-1k", "imagenet-a"], "Only imagenet-1k and imagenet-a are supported for now."

    # EVALUATION_MODE = "generation" # generation or classification
    EVALUATION_MODE = "classification"
    assert EVALUATION_MODE in ["generation",
                               "classification"], "Only generation and classification are supported for now."
    ...
    eval_loader = get_val_data_loader(EVALUATE_DATASET, batch_size=BS, num_workers=4, number_of_classes=NUMBER_OF_CLASSES)
    ...
    if to_train:
        for epoch in trange(epochs, desc="Epochs"):
            ...
            for batch in tbar:
                optimizer.zero_grad()
                vision_x, lang_x = prepare_one_training_batch(batch,
    ...
    for batch in tbar:
        vision_x, lang_x, batch_label = prepare_one_eval_batch(
            batch, NUMBER_OF_MEDIA_PROMPTS, NUMBER_OF_TEXT_PROMPTS_PER_MEDIA, tokenizer,
            image_processor,
            use_robust_prompting=USE_ROBUST_PROMPTING,
            robust_scales=ROBUST_SCALEs
        )
        ...
        if EVALUATION_MODE == "generation":
            ...
            raise NotImplementedError("Only generation and classification are supported for now.")
        total += len(batch_label)
        for b in range(len(batch_label)):
            if EVALUATION_MODE == "generation":
                generated_text = tokenizer.decode(generation[b])
                prediction = generated_text.split("<image>Output:")[-1].replace("<|endofchunk|>", "").strip()
