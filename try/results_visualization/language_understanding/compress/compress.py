...
coco_fig = sns.barplot(
    ...
    y="performance",
    hue="settings",
    palette=[COLOR_2, COLOR_3, COLOR_5, COLOR_5],
)
# coco_fig.axhline(y=80.189, color=COLOR_7, linestyle="--")
coco.set_title("MSCOCO", fontsize=14, fontfamily="times new roman")
coco_fig.set(xlabel="Number of shots", ylim=(0, 120), ylabel=None)
coco_fig.legend([], [], frameon=False)
# coco_fig.legend(loc="upper left", bbox_to_anchor=(1, 1))

# plt.legend(fontsize=2)
sns.move_legend(okvqa, "upper left", bbox_to_anchor=(1, 1), fontsize=8)

# fig.suptitle(f"Performance given different language information", fontsize=16, fontfamily="times new roman")
plt.show()

# save the plot
fig.savefig(f"understand-language-{MODEL}.pdf", dpi=500)